...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        User antonAfterLogin = null;
        try {
            antonAfterLogin = authenticationService.login("anton@net", "qwerty123");
            System.out.println(anton.equals(antonAfterLogin));
        } catch (AuthenticationException e) {
            throw new RuntimeException("Can not login user because of email or password", e);
        }

        try {
            authenticationService.register("anton@net", "123456");
        } catch (RegistrationException e) {
            throw new RuntimeException("Can not register user because of email or password", e);
        }
    }
